...
<div x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        ...
        <form method="GET" action="{{ route('admin.pengembalian') }}" class="flex items-center gap-3">
            <div class="relative">
            ...
            @if(request()->filled('search'))
                <a href="{{ route('admin.pengembalian') }}" class="rounded-xl bg-red-50 text-red-600 px-4 py-2 text-sm font-medium hover:bg-red-100 transition-colors focus:outline-none border border-red-100 flex items-center justify-center">
                    Reset
                </a>
            @endif
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-500 whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700">
                    ...
                    @php
                        $tenggat = \Carbon\Carbon::parse($item->tanggal_selesai)->endOfDay();
                        $sekarang = now();
                        $telatHari = 0;
                        if ($sekarang->greaterThan($tenggat)) {
                            $telatHari = $sekarang->diffInDays($tenggat) + 1;
                        }
                        $dendaPerHari = 50000;
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <span class="block font-medium text-gray-900 border-b border-gray-100 pb-1 mb-1">#{{ str_pad($item->idsewa, 4, '0', STR_PAD_LEFT) }}</span>
                            <span class="text-xs text-gray-500">{{ $item->user->nama ?? 'Unknown' }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            ...
                            @elseif($sekarang->isSameDay($tenggat))
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 rounded bg-green-50 px-2.5 py-1 text-xs font-medium text-green-600 ring-1 ring-inset ring-green-500/10">Aman</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <button @click="selectedData = {
                                    id_sewa: '{{ $item->idsewa }}',
                                    id_sewa_label: '#{{ str_pad($item->idsewa, 4, '0', STR_PAD_LEFT) }}',
                                    dendaTelat: '{{ number_format($dendaTelat, 0, ',', '.') }}',
                                    telat: '{{ $telatHari }} Hari',
                                    user: '{{ addslashes($item->user->nama ?? '') }}',
                                    wa: '{{ addslashes($item->user->no_hp ?? '') }}'
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada antrean validasi pengembalian saat ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-500 whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <span class="block font-medium text-gray-900 border-b border-gray-100 pb-1 mb-1">#{{ str_pad($item->id_sewa, 4, '0', STR_PAD_LEFT) }}</span>
                            <span class="text-xs text-gray-500 block">{{ $item->penyewaan->user->nama ?? 'Unknown' }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @if($item->keterlambatan_hari > 0)
                                <span class="inline-flex items-center gap-1.5 rounded bg-red-50 px-2.5 py-1 text-xs font-medium text-red-600 ring-1 ring-inset ring-red-500/10">Telat {{ $item->keterlambatan_hari }} Hari</span>
                            @else
                        ...
                        <td class="px-6 py-4">
                            @if($item->total_denda > 0)
                                <span class="font-medium text-red-600 block">Rp {{ number_format($item->total_denda, 0, ',', '.') }}</span>
                            @else
                                <span class="text-xs text-emerald-600 font-medium bg-emerald-50 px-2 py-1 rounded inline-block">Selesai / Tanpa Denda</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <span class="text-xs text-gray-700 block">{{ \Carbon\Carbon::parse($item->tanggal_kembali)->translatedFormat('d M Y') }}</span>
    ...
    <div x-show="qcModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/50 p-4 backdrop-blur-sm" x-cloak style="display: none;">
        <div @click.outside="qcModalOpen = false" class="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden" x-show="qcModalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 scale-100" x-transition:leave-end="opacity-0 translate-y-4 scale-95">
            ...
            <form action="{{ auth()->user()->role === 'admin' ? route('admin.pengembalian.validasi') : route('owner.pengembalian.validasi') }}" method="POST" class="p-6">
                @csrf
