...
<body class="pb-20">
    ...
    <nav class="bg-white border-b border-slate-100 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <a href="{{ route('keranjang.index') }}" class="flex items-center gap-2 text-slate-500 hover:text-primary transition-all font-bold uppercase italic text-xs">
            ...
            <div class="lg:col-span-7 space-y-8 text-left">
                <div>
                    ...
                    <div id="checkoutList" class="space-y-4">
                        ...
                        <div class="bg-white rounded-3xl p-4 border border-slate-100 flex items-center gap-4 shadow-sm hover:shadow-md transition-all">
                            ...
                            <div class="flex-1">
                                <h4 class="text-xs font-black text-slate-900 uppercase italic">{{ $item->alat->nama_alat }}</h4>
                                <p class="text-[10px] font-bold text-primary italic">Rp{{ number_format($item->alat->harga_sewa, 0, ',', '.') }} / hari</p>
                            </div>
                ...
                <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                    <div class="flex items-center gap-2 text-brand-500">
                        ...
                        <h4 class="text-xs font-black uppercase italic text-slate-900">Verifikasi Identitas</h4>
                    </div>
            ...
            <div class="lg:col-span-5 text-left">
                <div class="bg-white rounded-[2.5rem] p-8 cartoon-accent-shadow sticky top-24 space-y-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <input type="text" name="nama_penyewa" value="{{ $user_data->nama ?? '' }}" 
                            class="w-full bg-slate-100 border border-slate-200 rounded-2xl p-4 text-xs font-black outline-none italic cursor-not-allowed" 
                            readonly placeholder="Nama Lengkap">

                        <input type="text" name="telepon_penyewa" value="{{ $user_data->notelp ?? '' }}" 
                            class="w-full bg-slate-100 border border-slate-200 rounded-2xl p-4 text-xs font-black outline-none italic cursor-not-allowed" 
    ...
    <script>
        ...
        function updateLocationDetails(latlng) {
            ...
            const isDelivery = document.querySelector('input[name="metode_pengiriman"]:checked').value === "antar";
            if (isDelivery) {
        ...
        async function submitOrder() {
            ...
            if (document.querySelector('input[name="metode_pengiriman"]:checked').value === "antar") {
            ...
            try {
                const response = await fetch('{{ route("checkout.proses") }}', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    window.location.href = `/checkout/sukses/${result.idsewa}`;
                } else {
